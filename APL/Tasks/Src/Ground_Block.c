...
void Ground_Block_Func(CAN_RxHeaderTypeDef RxHeader, uint8_t *Rxdata)
{
    ...
    else if (solenoid_enable == 1 && DJmotor[GROUND_BLOCK_dji_num].Begin == 1)
    {
        ...
        if (RxHeader.ExtId == 0x01010302) gb_cmd = Ground_Block_GetReady_Flag;  
        else if (RxHeader.ExtId == 0x01010303) gb_cmd = Ground_Block_Fetch_Flag;  
        else if (RxHeader.ExtId == 0x01010304) gb_cmd = Ground_Block_Lay_Flag;  
        else if (RxHeader.ExtId == 0x010103FF) gb_cmd = Ground_Block_reset_Flag;  
    }
}
...
void Ground_Block_Process(void)
{
    ...
    switch (cmd)
    {
    case Ground_Block_GetReady_Flag: Ground_Block_GetReady();      break;
    case Ground_Block_Fetch_Flag: Ground_Block_Fetch(gb_data);  break;
    case Ground_Block_Lay_Flag: Ground_Block_Lay(gb_data);    break;
    case Ground_Block_reset_Flag: Ground_Block_reset();         break;
    }
}
